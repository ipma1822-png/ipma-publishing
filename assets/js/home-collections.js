(function(){
  ...
  fetch(CSV_URL,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('load');return r.text();}).then(text=>{
    ...
    if(latestGrid){const latest=records.filter(r=>/^\d{4}-\d{2}-\d{2}$/.test(r.publish_date||'')).sort((a,b)=>b.publish_date.localeCompare(a.publish_date)||Number(a.source_row||9999)-Number(b.source_row||9999)).slice(0,6);latestGrid.innerHTML=latest.length?latest.map(latestCard).join(''):'<div class="featured-loading">최근 발행 출판물 정보를 찾지 못했습니다.</div>';}
  }).catch(()=>{
